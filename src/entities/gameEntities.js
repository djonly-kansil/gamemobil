...
export class GameEntities {
  ...
  update() {
    ...
    if (frame % 60 === 0 && this.obstacles.length < 20) {
      let laneIndex, attempts = 0;
      do {
        laneIndex = Math.floor(Math.random() * this.laneCount);
        const testObstacle = new Obstacle(canvas, this.laneCount, laneIndex);
        attempts++;
      } while (
        this.game.isOverlapping(
          canvas.width / this.laneCount * laneIndex + this.laneWidth / 2,
          16,
          32, 32,
          [...this.obstacles, ...this.points, ...this.game.powerUps.powerUps]
        ) && attempts < 50
      );
      if (attempts < 50) {
        this.obstacles.push(new Obstacle(canvas, this.laneCount, laneIndex));
      }
    }
    ...
    this.obstacles.forEach(ob => ob.update(speedMultiplier, bossMode, Date.now()));
